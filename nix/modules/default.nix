...

{
  ...
  nix.settings = {
    # cuda-maintainers serves the cudaSupport = true variants of the CUDA
    # closure (torch, vllm, llama-cpp). nixpkgs' own Hydra builds everything
    # with cudaSupport off and cache.nixos.org cannot redistribute the CUDA
    # ones, so without this every GPU package compiles locally.
    substituters = [
